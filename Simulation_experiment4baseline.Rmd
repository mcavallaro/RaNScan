...
set.seed(1)
start_time <- Sys.time()
cylinders = ranScanCreateCylinders(observation.matrix = sim2, baseline.matrix = b.matrix, emmtype = 'sim', week.range = c(0,99), n.cylinders = 100000, coord.df=df.cases2,  size_factor = 1.2)

set.seed(1)
cylinders2 = ranScanCreateCylinders(observation.matrix = sim2, baseline.matrix =  b.matrix, emmtype = 'sim', week.range = c(0,99), n.cylinders = 100000, coord.df=df.cases2,  size_factor = 1.5)
...
png(paste0('Fig/satscan_',label.str,'.png'), width =  4 *1.2, height = 3 *1.2, units = 'in', res=400, pointsize = 17)
par(mfrow = c(1, 1), mar=c(1,1,1,1)+0.1)
plot(case.df$longitude, case.df$latitude, col='#1f77b4',axes=F, pch=1, cex=1, ylab = NA, xlab=NA,  xlim=c(-0.65,0.3), ylim=c(51.4,51.8) )
idx3=case.df$warning.score > 0.95
points(case.df[idx3,]$longitude, case.df[idx3,]$latitude, col='#ff7f0e', pch=20, cex=1, ylab = NA, xlab=NA)
points(case.df[case.df$true_positive,]$longitude, case.df[case.df$true_positive,]$latitude, col='#d62728', pch=4, cex=1, ylab = NA, xlab=NA)
plotBaseMap(add=T, Wales = F)
sim.col=st_read('Data/satscan_output.shp')
...
points(case.df[idx2,]$longitude, case.df[idx2,]$latitude, col='#ff7f0e', pch=20, cex=1, ylab = NA, xlab=NA)
plotBaseMap(add=T, Wales = F)
legend('topleft', c( expression(italic(w)>=0.95),expression(italic(w)<0.95)), col=c("#ff7f0e", "#1f77b4"), pch=c(20,1))
dev.off()
```
...
axis(1, at = seq(40,59))
mtext('D', side=3, padj=3, at=42, cex=1.5)

plot(case.df$warning.score, case.df$warning.score2,
     xlab = expression(paste(italic(w),' (rep. 1)')), ylab = expression(paste(italic(w)," (rep. 2)")), col=colors, pch=ifelse(case.df$true_positive, 20,1))
legend('bottomright', c('epidemic','endemic'), col=c("#d6272855", "#1f77b455"), pch=c(20,1))

mtext('E', side=3, padj=2, at=(42-40)/(58.5-40), cex=1.5)

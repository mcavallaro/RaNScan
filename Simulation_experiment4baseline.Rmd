...
Aggregate epidemic and endemic cases:
```{r}
sim2 = sim
sim2[1:sum(idx1), as.character(40:59)] = sim2[1:sum(idx1), as.character(40:59)] + epi.matrix
```

...and plot. This shows that is essentially impossible to visually detect the epidemic element from these aggregate data. The following code requires UK boundaries shape files, not sure if sharing is allowed.
...
Apply RanNCover. The matrix data must be converted to list data (as in the iGAS dataset).
```{r}
...
set.seed(1)
start_time <- Sys.time()
cylinders = CreateCylinders(observation.matrix = sim2, baseline.matrix = b.matrix, emmtype = 'sim', week.range = c(0,99), n.cylinders = 100000, coord.df=df.cases2,  size_factor = 1.2)

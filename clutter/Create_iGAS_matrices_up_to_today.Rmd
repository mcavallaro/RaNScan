...
Transform this document into R file.
...
for(i in 1:dim(observation.matrix)[1]){
  for(j in 1:dim(observation.matrix)[2]){
    for(k in 1:dim(observation.matrix)[3]){ #time
      baseline.matrix.2[i,j,k] <- time.factor[k] * emmtype.factor[k, j] * location.factor[k, i]
    }
  }
}
# cat(sprintf("%d\r", i))
baseline.matrix.2 = baseline.matrix.2 * sum(observation.matrix) / sum(baseline.matrix.2)
save("baseline.matrix.2", file="iGAS_baseline_matrix_2_to_day.Rdata")
...
save("baseline.matrix.3", file="iGAS_baseline_matrix_3_to_day.Rdata")
```
...
save("baseline.matrix.4", file="iGAS_baseline_matrix_4_to_day.Rdata")
```
...
```{r}
png('Fig/time_factor.png', res=600, width=5, height=4, units="in")
time.series = table(case.df$SAMPLE_DT_numeric)
plot(time.series[-1], xlab = 'Date', ylab='', xaxt='n', type = 's', lwd=1)
...
legend("topleft", pch=c(NA,NA), lty = c(1, 1), lwd=c(1, 1.5) , col=c('black', 'red'), c("No. of cases", "Temporal baseline"))

...
For each postcode let's plot the population vs the total number of cases:

```{r}
Cases = as.data.frame(table(dat2$`Patient Postcode`))
Population = read.table("Data/Postcode_Estimates_Table_1.csv", header=T, sep = ',', stringsAsFactors = F)
tmp = merge(Cases, Population, by.x='Var1', by.y = 'Postcode')
```



```{r}
plot(tmp$Total, tmp$Freq, log='xy', ylab='cases', xlab = 'population')
Lm=lm(Freq~Total, tmp)
points(tmp$Total,predict(Lm, newdata=tmp),col='red')
```
